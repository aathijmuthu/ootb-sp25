...
sessions_anomaly_percentages_source = sessions_source_contributions.set_index('ds')[["percent_diff " + source for source in source_list]]
sessions_anomaly_percentages_medium = sessions_medium_contributions.set_index('ds')[["percent_diff " + medium for medium in medium_list]]
sessions_medium_contributions
...
# %%
sessions_source_contributing_columns = create_contributor_col(sessions_anomaly_percentages_source)
print(sessions_anomaly_percentages_source.columns)
sessions_source_contributing_columns.reset_index(inplace=True)
sessions_source_contributing_columns.head()

sessions_medium_contributing_columns = create_contributor_col(sessions_anomaly_percentages_medium)
...
print(sessions_source_contributions.columns)
...
def analyze_historical_changes(group_report):
    """
    Analyze historical changes in metrics by comparing with previous day and week values.
    Uses the original metric dataframes for historical comparisons.
    Includes analysis of all dimensions for each metric.
    
    Args:
        group_report (pd.DataFrame): The anomaly group report dataframe
        
    Returns:
        pd.DataFrame: A dataframe containing the historical analysis results
    """
    # Create a copy of the report to work with
    df = group_report.copy()
    
    # Initialize lists to store results
    results = []
    
    # Define the metric dataframes mapping with their dimensions
    metric_dataframes = {
        'landing_page_viewers': {
            'top_level': landing_page_viewers_top_level,
            'source': sessions_source_contributions,
    ...
    # Process each anomaly group
    for _, row in df.iterrows():
        start_hour = row['start_hour']
        end_hour = row['end_hour']
        
        # Calculate previous day and week times
        prev_day_start = start_hour - pd.Timedelta(days=1)
        prev_day_end = end_hour - pd.Timedelta(days=1)
        prev_week_start = start_hour - pd.Timedelta(weeks=1)
        prev_week_end = end_hour - pd.Timedelta(weeks=1)
        ...
        # Create result entry
        result = {
            'anomaly_period': f"{start_hour} to {end_hour}",
            'prev_day_period': f"{prev_day_start} to {prev_day_end}",
            'prev_week_period': f"{prev_week_start} to {prev_week_end}",
            'metrics_analysis': []
        }
        
        # Analyze each metric
        for metric_col in metric_columns:
            metric_name = metric_col.replace('_percent_diff', '')
            
            # Get the corresponding dataframes for this metric
            metric_data = metric_dataframes.get(metric_name)
            if metric_data is None:
                continue
            
            # Initialize metric analysis
            metric_analysis = {
                'metric': metric_name,
                'dimensions': {}
            }
            
            # Analyze top level metric
            top_level_df = metric_data['top_level']
            current_period_data = top_level_df[
                (top_level_df['ds'] >= start_hour) & 
                (top_level_df['ds'] <= end_hour)
            ]
            current_avg = current_period_data['percent_diff'].mean() * 100 if not current_period_data.empty else None
            
            prev_day_data = top_level_df[
                (top_level_df['ds'] >= prev_day_start) & 
                (top_level_df['ds'] <= prev_day_end)
            ]
            prev_day_avg = prev_day_data['percent_diff'].mean() * 100 if not prev_day_data.empty else None
            
            prev_week_data = top_level_df[
                (top_level_df['ds'] >= prev_week_start) & 
                (top_level_df['ds'] <= prev_week_end)
            ]
            prev_week_avg = prev_week_data['percent_diff'].mean() * 100 if not prev_week_data.empty else None
            
            # Calculate differences between percent changes
            prev_day_change = current_avg - prev_day_avg if prev_day_avg is not None else None
            prev_week_change = current_avg - prev_week_avg if prev_week_avg is not None else None
            
            # Determine if the change is significant
            def is_significant_change(change):
                if change is None:
                    return "No data"
                return "Significant" if abs(change) > 10 else "Similar"
            
            metric_analysis['top_level'] = {
                'current_value': current_avg,
                'prev_day_value': prev_day_avg,
                'prev_week_value': prev_week_avg,
                'vs_prev_day': is_significant_change(prev_day_change),
                'vs_prev_week': is_significant_change(prev_week_change),
                'prev_day_change_pct': prev_day_change,
                'prev_week_change_pct': prev_week_change
            }
            
            # Analyze dimensions
            for dim_type, dim_df in metric_data.items():
                if dim_type == 'top_level':
                    continue
                
                # Get timestamp column name
                time_col = 'ds' if 'ds' in dim_df.columns else 'time_truncated'
                
                # Get all dimension columns
                dim_columns = [col for col in dim_df.columns if col.startswith('percent_diff') and col != 'percent_diff']
                
                for dim_col in dim_columns:
                    dim_name = dim_col.replace('percent_diff ', '')
                    
                    # Get values for current period
                    current_period_data = dim_df[
                        (dim_df[time_col] >= start_hour) & 
                        (dim_df[time_col] <= end_hour)
                    ]
                    current_avg = current_period_data[dim_col].mean() * 100 if not current_period_data.empty else None
                    
                    # Get values for previous day
                    prev_day_data = dim_df[
                        (dim_df[time_col] >= prev_day_start) & 
                        (dim_df[time_col] <= prev_day_end)
                    ]
                    prev_day_avg = prev_day_data[dim_col].mean() * 100 if not prev_day_data.empty else None
                    
                    # Get values for previous week
                    prev_week_data = dim_df[
                        (dim_df[time_col] >= prev_week_start) & 
                        (dim_df[time_col] <= prev_week_end)
                    ]
                    prev_week_avg = prev_week_data[dim_col].mean() * 100 if not prev_week_data.empty else None
                    ...
                    if dim_type not in metric_analysis['dimensions']:
                        metric_analysis['dimensions'][dim_type] = {}
                    metric_analysis['dimensions'][dim_type][dim_name] = dim_analysis
            
            result['metrics_analysis'].append(metric_analysis)
    ...
    # Save to CSV
    analysis_df.to_csv('historical_analysis_report.csv', index=False)
    
    # Print the analysis in a readable format
    for result in results:
        print(f"\nAnomaly Period: {result['anomaly_period']}")
        print(f"Previous Day Period: {result['prev_day_period']}")
        print(f"Previous Week Period: {result['prev_week_period']}")
        print("\nMetrics Analysis:")
        print("================")
        
        for metric_analysis in result['metrics_analysis']:
            metric_name = metric_analysis['metric']
            print(f"\nMetric: {metric_name}")
            
            # Print top level analysis
            top_level = metric_analysis['top_level']
            print("  Top Level:")
            print(f"    Current Value: {top_level['current_value']:.2f}%" if top_level['current_value'] is not None else "    Current Value: No data")
            
            # Format previous day comparison
            if top_level['prev_day_value'] is not None and top_level['current_value'] is not None:
                prev_day_diff = top_level['current_value'] - top_level['prev_day_value']
                print(f"    Previous Day: {top_level['prev_day_value']:.2f}% (Diff: {prev_day_diff:+.2f}%) - {top_level['vs_prev_day']}")
            else:
                print("    Previous Day: No data")
            
            # Format previous week comparison
            if top_level['prev_week_value'] is not None and top_level['current_value'] is not None:
                prev_week_diff = top_level['current_value'] - top_level['prev_week_value']
                print(f"    Previous Week: {top_level['prev_week_value']:.2f}% (Diff: {prev_week_diff:+.2f}%) - {top_level['vs_prev_week']}")
            else:
                print("    Previous Week: No data")
            
            # Print dimension analysis
            for dim_type, dimensions in metric_analysis['dimensions'].items():
                print(f"\n  {dim_type.title()} Dimensions:")
                for dim_name, dim_analysis in dimensions.items():
                    print(f"    {dim_name}:")
                    print(f"      Current Value: {dim_analysis['current_value']:.2f}%" if dim_analysis['current_value'] is not None else "      Current Value: No data")
                    
                    # Format previous day comparison
                    if dim_analysis['prev_day_value'] is not None and dim_analysis['current_value'] is not None:
                        prev_day_diff = dim_analysis['current_value'] - dim_analysis['prev_day_value']
                        print(f"      Previous Day: {dim_analysis['prev_day_value']:.2f}% (Diff: {prev_day_diff:+.2f}%) - {dim_analysis['vs_prev_day']}")
                    else:
                        print("      Previous Day: No data")
                    
                    # Format previous week comparison
                    if dim_analysis['prev_week_value'] is not None and dim_analysis['current_value'] is not None:
                        prev_week_diff = dim_analysis['current_value'] - dim_analysis['prev_week_value']
                        print(f"      Previous Week: {dim_analysis['prev_week_value']:.2f}% (Diff: {prev_week_diff:+.2f}%) - {dim_analysis['vs_prev_week']}")
                    else:
                        print("      Previous Week: No data")
...
historical_analysis = analyze_historical_changes(group_report)
print("\nHistorical Analysis Report:")
print("==========================")
for _, row in historical_analysis.iterrows():
    print(f"\nAnomaly Period: {row['anomaly_period']}")
    print(f"Previous Day Period: {row['prev_day_period']}")
    print(f"Previous Week Period: {row['prev_week_period']}")
    print("\nMetrics Analysis:")
    for metric in row['metrics_analysis']:
        print(f"\n{metric['metric']}:")
        
        # Print top level analysis
        top_level = metric['top_level']
        print("  Top Level:")
        print(f"    Current Value: {top_level['current_value']:.2f}%" if top_level['current_value'] is not None else "    Current Value: No data")
        ...
        print(f"    vs Previous Day: {top_level['vs_prev_day']}")
        print(f"    vs Previous Week: {top_level['vs_prev_week']}")
        
        # Print dimension analysis
        for dim_type, dimensions in metric['dimensions'].items():
            print(f"\n  {dim_type.title()} Dimensions:")
            for dim_name, dim_analysis in dimensions.items():
                print(f"    {dim_name}:")
                print(f"      Current Value: {dim_analysis['current_value']:.2f}%" if dim_analysis['current_value'] is not None else "      Current Value: No data")
                ...
                print(f"      vs Previous Day: {dim_analysis['vs_prev_day']}")
                print(f"      vs Previous Week: {dim_analysis['vs_prev_week']}")
