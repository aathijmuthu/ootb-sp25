...
def generate_anomaly_report():
    ...
    metrics = {
        'landing_page_viewers': {
            ...
            'contributing_cols': sessions_contributing_columns
        },
        'visitors': {
            'top_level': visitors_top_level,
            'geo': visitors_geo_contributions,
            'device': visitors_contributions_device,
            'source': None,
            'medium': None,
            'contributing_cols': visitor_contributing_columns
        },
        'product_viewers': {
            'top_level': product_viewers_top_level,
            'geo': product_viewers_geo_contributions,
            'device': product_viewers_contributions_device,
            'source': None,
            'medium': None,
            'contributing_cols': product_viewers_contributing_columns
        },
        'added_to_cart': {
            'top_level': added_to_cart_top_level,
            'geo': added_to_cart_geo_contributions,
            'device': added_to_cart_contributions_device,
            'source': None,
            'medium': None,
            'contributing_cols': added_to_cart_contributing_columns
        },
        'checkout_started': {
            'top_level': checkout_started_top_level,
            'geo': checkout_started_geo_contributions,
            'device': checkout_started_contributions_device,
            'source': None,
            'medium': None,
            'contributing_cols': checkout_started_contributing_columns
        },
        'orders': {
            'top_level': orders_top_level,
            'geo': orders_geo_contributions,
            'device': orders_contributions_device,
            'source': None,
            'medium': None,
            'contributing_cols': order_contributing_columns
        },
        'buyers': {
            'top_level': buyers_top_level,
            'geo': buyers_geo_contributions,
            'device': buyers_contributions_device,
            'source': None,
            'medium': None,
            'contributing_cols': buyer_contributing_columns
        }
    }
    ...
    for metric_name, metric_data in metrics.items():
        try:
            ...
            if metric_data['device'] is not None:
                for device in ['iOS', 'Android', 'Windows', 'macOS', 'Linux', 'Other']:
            ...
            # Add contributors for each dimension
            dimensions = {
                'geo': ['Global', 'US'] if metric_data['geo'] is not None else [],
                'device': ['iOS', 'Android', 'Windows', 'macOS', 'Linux', 'Other'] if metric_data['device'] is not None else [],
                'source': ['google', 'fbig', 'Klaviyo', 'rakuten', 'tiktok'] if metric_data['source'] is not None else [],
                'medium': ['cpc', 'paid_social', 'email', 'affiliates'] if metric_data['medium'] is not None else []
            }
            
            for dim_type, dim_values in dimensions.items():
                for dim_value in dim_values:
                    col_name = f'{metric_name}_{dim_type}_{dim_value}_contributors'
                    report_df[col_name] = None
                    
                    for idx, row in report_df.iterrows():
                        hour = row['hour']
                        contrib_data = metric_data['contributing_cols'][metric_data['contributing_cols']['ds_x'] == hour]
                        
                        if len(contrib_data) > 0:
                            contributors = clean_contributors(contrib_data['Contributors'].tolist())
                            if contributors:
                                # Filter contributors for this specific dimension value
                                dim_contributors = [c for c in contributors if dim_value.lower() in c.lower()]
                                if dim_contributors:
                                    report_df.at[idx, col_name] = dim_contributors
